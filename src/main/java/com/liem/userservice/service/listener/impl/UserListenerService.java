...
public class UserListenerService
   ...
   */
  protected final AuthenticateService<String> authenticateService;
  ...
  public UserListenerService(UserQueryService queryService,
      ...
      AuthenticateService<String> authenticateService) {
  ...
  protected BaseMessage<UserDto<Long>> handleAuthenticate(
    ...
    try {
      final var token = this.authenticateService.authenticate(username, password);
      userDtoBaseMessage.getBody().setToken(token);
      userDtoBaseMessage.setHttpStatus(HttpStatus.OK);
      userDtoBaseMessage.setOrigin(this.createResponseOrigin());
      return userDtoBaseMessage;
    } catch (Exception exception) {
      exception.printStackTrace();
      return this.handleFailedAuthenticate(userDtoBaseMessage, exception.getMessage());
    }
  }
